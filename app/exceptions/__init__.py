...
def configure_exception_handler(app):
    ...
    def handle_http_exception(e):
        ...
        return to_json_dict({"status": "error", "message": str(e)}), e.code

    @app.errorhandler(HTTPError)
    def handle_http_exception(e):
        current_app.logger.exception(str(e))
        error_message = "Unauthorized" if e.response.status_code==401 else str(e)
        return to_json_dict({"status": "error", "message": error_message}), e.response.status_code

    @app.errorhandler(Exception)
    def handle_exception(e):
        current_app.logger.exception(str(e))
        return to_json_dict({"status": "error", "message": str(e)}), 500

    @app.errorhandler(InvalidDataError)
    def handle_exception(e):
        current_app.logger.exception(e.message)
        return to_json_dict({"status": "error", "message": e.message, "data": e.data}), e.status_code or 400
...
class ClientError(HTTPError):
    def __init__(self, message=None, status_code=None, data={}):
        Exception.__init__(self)
        self.message = message or 'Client Error'
        self.status_code = status_code or 400
        self.data = data
